...
class EamFSSetflBuilder():
    """
    Class supporting the construction of LAMMPS eam/fs setfl parameter files     
    """
    ...
    def set_rho_function(self, symbols, function, **kwargs):
        """
        ...
        **kwargs : any
        ...
        self.__rho_function[symbolstr] = function
        self.__rho_kwargs[symbolstr] = kwargs
    ...
    def rho(self, symbols, r):
        ...
        ----------
        symbol : str
            The two elements symbol associated with the rho(r) function to use.
        ...
        symbolstr = '-'.join(symbols)
        try:
            function = self.__rho_function[symbolstr]
            kwargs = self.__rho_kwargs[symbolstr]
        except:
            raise KeyError(f'rho function not set for {symbols}')
    ...
    def build(self, numr=10000, cutoffr=6.0, numrho=10000, cutoffrho=500.0,
        ...
        for symbol in self.__symbol:
            ...
            for symbol2 in self.__symbol:
                vals.append(self.rho([symbol, symbol2], r))
            vals = np.hstack(vals)
